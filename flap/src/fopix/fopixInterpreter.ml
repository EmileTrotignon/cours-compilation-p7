...
let print_value m v =
  ...
  let rec print_value d v =
    if d >= max_depth then "..."
      ...
      | VUnit           -> "()"
      | VAddress a      -> print_block m d a
      | VFun (FunId id) -> Printf.sprintf "<fun: %s>" id
...
module Environment : sig
  ...
  let lookup x e =
    try List.assoc x e with Not_found -> raise (UnboundIdentifier x)

  let last = function [] -> None | (x, v) :: e -> Some (x, v, e)
  ...
  let print_binding memory (Id x, v) =
    ...
    if x.[0] = '_' then "" else x ^ " = " ^ print_value memory v
...
type observable = { new_environment : Environment.t }
...
let initial_runtime () =
  ...
  let bind_unit s env = Environment.bind env (Id s) VUnit in
  {
    ...
    environment =
      Environment.initial |> bind_bool "true" true |> bind_bool "false" false
      |> bind_unit "nothing";
    functions = [];
  }
...
and expression runtime = function
  | Literal l -> literal l
  | Variable (Id "true") -> VBool true
  | Variable (Id "false") -> VBool false
  ...
  | While (cond, e) ->
      ...
      VUnit
  | Switch (e, bs, default) -> (
      match value_as_int (expression runtime e) with
  ...
  | FunCall (FunId "allocate_block", [ size ]) -> (
      match value_as_int (expression runtime size) with
      | Some size ->
          let a = Memory.allocate runtime.memory size VUnit in
          VAddress a
      | None      -> error [] "A block size should be an integer." )
  ...
  | UnknownFunCall (e, arguments) -> (
      match expression runtime e with
      | VFun f -> expression runtime (FunCall (f, arguments))
      | v      ->
          failwith
            (Printf.sprintf "By construction: %s"
               (print_value runtime.memory v))
          (*assert false*)
          (* By construction. *) )
...
and binop :
 ...
 fun m kind coerce wrap runtime op l r ->
  let lv = expression runtime l and rv = expression runtime r in
  match (coerce lv, coerce rv) with
  | Some li, Some ri -> wrap (op li ri)
  | _, _             ->
      error []
        (Printf.sprintf "Invalid %s binary operation between %s and %s." kind
           (print_value m lv) (print_value m rv))

...
type imm = Lit of lit | Lab of label
...
type dst = [ `Addr of address | `Reg of reg ]

type src = [ dst | `Imm of imm ]
...
let int_of_scale s =
  match s with `Zero -> 0 | `One -> 1 | `Two -> 2 | `Four -> 4 | `Eight -> 8

(* Helper functions *)

let lit i = `Imm (Lit i)

let liti i = lit (Mint.of_int i)

let addr ?offset ?idx ?(scale = `One) ?base () = { offset; base; idx; scale }

let addq ~src ~dst = Add { s = `q; src; dst }

let subq ~src ~dst = Sub { s = `q; src; dst }

let imulq ~src ~dst = Imul { s = `q; src; dst }

let idivq ~src = Idiv { s = `q; src }

let andq ~src ~dst = And { s = `q; src; dst }

let orq ~src ~dst = Or { s = `q; src; dst }

let xorq ~src ~dst = Xor { s = `q; src; dst }

let notq ~dst = Not { s = `q; dst }

let leaq ~src ~dst = Lea { s = `q; dst; src }

let incq ~dst = Inc { s = `q; dst }

let decq ~dst = Dec { s = `q; dst }

let cmpq ~src1 ~src2 = Cmp { s = `q; src1; src2 }

let pushq ~src = Push { s = `q; src }

let popq ~dst = Pop { s = `q; dst }

let pushr ~reg = Push { s = `q; src = `Reg reg }

let popr ~reg = Pop { s = `q; dst = `Reg reg }

let movq ~src ~dst = Mov { s = `q; src; dst }

let calld ~tgt = CallD { tgt }

let calli ~tgt = CallI { tgt }

let calldi ~tgt =
  match tgt with `Imm tgt -> calld ~tgt | #dst as tgt -> calli ~tgt

let calll ~tgt = calld ~tgt:(Lab tgt)

let jmpd ~tgt = JmpD { tgt }

let jmpi ~tgt = JmpI { tgt }

let jmpdi ~tgt =
  match tgt with `Imm tgt -> jmpd ~tgt | #dst as tgt -> jmpi ~tgt

let jmpl ~tgt = jmpd ~tgt:(Lab tgt)

let jcc ~cc ~tgt = Jcc { cc; tgt }

let jccl ~cc ~tgt = jcc ~cc ~tgt:(Lab tgt)

let cmovq ~cc ~src ~dst = Cmov { s = `q; cc; src; dst }

let cqto = Ct { s = `q }

let insns = List.map (fun i -> Instruction i)

let string s = Directive (String s)

let text_section = Directive (Section "text")

let data_section = Directive (Section "data")
...
let referenced_labels_imm imm refs =
  match imm with Lit _ -> refs | Lab l -> LabSet.add l refs
...
let referenced_labels_ins ins refs =
  ...
  | Idiv { src } | Push { src } -> referenced_labels_src src refs
  | Cmp { src1; src2 } ->
      referenced_labels_src src1 @@ referenced_labels_src src2 refs
  | Lea { src; dst } ->
      referenced_labels_addr src @@ referenced_labels_dst dst refs
  | CallD { tgt } | JmpD { tgt } | Jcc { tgt } -> referenced_labels_imm tgt refs
  | CallI { tgt } | JmpI { tgt } -> referenced_labels_dst tgt refs
  | Ret | Comment _ | Ct _ -> refs

let referenced_labels_directive d refs =
  match d with
  | Quad imms ->
      List.fold_left (fun refs imm -> referenced_labels_imm imm refs) refs imms
  | Global l -> LabSet.add l refs
  | Section _ | Extern _ | String _ | PadToAlign _ -> refs

let referenced_labels_line refs line =
  match line with
  | Directive d     -> referenced_labels_directive d refs
  | Label _         -> refs
  | Instruction ins -> referenced_labels_ins ins refs
...
let zeroq dst = xorq ~src:(dst :> src) ~dst
